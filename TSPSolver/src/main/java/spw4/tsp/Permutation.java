...
public class Permutation {
    ...
    public Permutation(int length) {
        if (length <= 1) throw new IllegalArgumentException("length of permutation must be larger than 1");
    ...
    public static Permutation createRandom(int length) {
        if (length <= 1) throw new IllegalArgumentException("length of random permutation must be larger than 1");
        ...
        for (int i = 0; i < length - 1; i++) {
            //var result = TSPSolver.random.nextInt(length - i);
